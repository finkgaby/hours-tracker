...
def get_target_hours(dt):
    wd = dt.weekday()
    # ראשון-רביעי: 9, חמישי: 8.5, שישי-שבת: 0
...
def get_adjusted_weekly_target(data_df):
    ...
    # מציאת יום ראשון הקרוב ביותר בעבר
...
with tab_report:
    ...
    if st.button("שמור דיווח", type="primary", use_container_width=True):
        new_row = pd.DataFrame([{"date": str(d_in), "start_time": str(st_t), "end_time": str(en_t), "notes": "", "type": r_type}])
